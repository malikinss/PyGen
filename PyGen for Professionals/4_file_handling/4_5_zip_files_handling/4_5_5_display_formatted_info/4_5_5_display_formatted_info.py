...
def get_formatted_info(files: List[ZipInfo]) -> Dict[str, List[str]]:
    ...
    # Sort the items and return as a new dictionary
    return dict(sorted(formatted_info.items(), key=lambda item: item[0]))
...
def display_formatted_info(formatted_info: Dict[str, List[str]]) -> None:
    ...
    """
    # English equivalents of the prefixes
    prefixes = [
    ...
    for name, value in formatted_info.items():
        print(
            name,
            f'  {prefixes[0]}: {value[0]}',
            f'  {prefixes[1]}: {value[1]} bytes',
            f'  {prefixes[2]}: {value[2]} bytes',
            sep='\n',
            end='\n'
        )
