...
def extract_ids(csv_data: List[List[str]]) -> List[str]:
    ...
    """
    unique_ids = set(row[0] for row in csv_data)
    return list(unique_ids)


def extract_headers(
    csv_data: List[List[str]], id_column_name: str
) -> List[str]:
    """
    ...
    headers.extend({row[1] for row in csv_data})  # use set to avoid duplicates
    return list(headers)


def create_records(
    csv_data: List[List[str]], headers: List[str], unique_ids: List[str]
) -> List[Dict[str, str]]:
    ...
    Args:
        ...
        unique_ids (List[str]): The list of unique IDs.

    Returns:
        List[Dict[str, str]]: A list of records as dictionaries.
    """
    records = {id_: {headers[0]: id_} for id_ in unique_ids}
...
def condense_csv(filename: str, id_name: str) -> None:
    """
    Condenses a CSV file into a summarized format and writes the result to
    a new file.

    Args:
        filename (str): The path to the input CSV file.
        id_name (str): The name to use for the ID column in the output
                       CSV file.
    """
    csv_data = read_csv_file(filename)
    unique_ids = extract_ids(csv_data)
    headers = extract_headers(csv_data, id_name)
    records = create_records(csv_data, headers, unique_ids)
