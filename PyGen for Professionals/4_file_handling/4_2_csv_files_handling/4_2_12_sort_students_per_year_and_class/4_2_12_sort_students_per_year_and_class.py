...
def read_csv(filename: str,
    ...
    try:
        ...
        raise Exception(f"The file {filename} was not found.")
    ...
    try:
        with open(filename, 'w', encoding='utf-8', newline='') as file:
            writer = csv.DictWriter(
                file, fieldnames=columns, delimiter=delimiter
            )
            writer.writeheader()
            writer.writerows(data)
    except Exception as e:
        raise Exception(f"An error occurred while writing the file: {e}")
...
def sort_headers(headers: List[str]) -> List[str]:
    ...
    for header in headers:
        if header == 'year':
            ...
            # Sorting by class number and letter
            # (first numeric then alphabetic)
            # Extract numeric part
            class_number = ''.join(filter(str.isdigit, header))
...
def sort_rows(rows: List[Dict[str, str]],
    ...
    """
    return [{header: row[header] for header in sorted_headers} for row in rows]


def sort_and_rearrange_data(
    filename: str
...
sorted_data, sorted_headers = sort_and_rearrange_data(FILE_TO_READ)
