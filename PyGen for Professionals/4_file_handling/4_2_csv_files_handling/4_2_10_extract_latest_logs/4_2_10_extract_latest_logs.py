...
TODO:
    ...
    user from the name_log.csv file and writes them to the new_name_log.csv
    file.
...
def find_latest_date(dates: List[str]) -> dt:
    ...
    """
    return max(
        (dt.strptime(date, TIME_MASK) for date in dates), key=lambda x: x
    )
...
def find_latest_user_log(logs: List[Dict[str, str]]) -> Dict[str, str]:
    ...
    """
    latest_date = find_latest_date([log['dtime'] for log in logs])
    return next(
        log for log in logs
        if dt.strptime(log['dtime'], TIME_MASK) == latest_date
    )
