...
def convert_meeting_to_datetime(meeting: Meeting) -> datetime:
    ...
    Args:
        meeting (Meeting): A namedtuple representing a meeting with
                            surname, name, meeting_date, and meeting_time.

    Returns:
        datetime: A datetime object representing the combined date and time.
    """
    str_date = f'{meeting.meeting_date} {meeting.meeting_time}'
    time_format = '%d.%m.%Y %I:%M %p'  # Adjusted to handle AM/PM format
...
def create_sorted_meetings(data: List[Dict[str, str]]) -> List[Meeting]:
    ...
    Args:
        data (List[Dict[str, str]]): A list of dictionaries containing meeting
        data.
    ...
    meetings = [
        Meeting(
            ...
            record['meeting_date'], record['meeting_time']
        )
        for record in data
    ]
    return sorted(meetings, key=convert_meeting_to_datetime)
