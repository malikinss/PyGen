...
def filter_numbers(data: List) -> filter:
    """
    ...
    Returns:
        filter: A filter object with only integers and floats.
    """
    return filter(lambda x: isinstance(x, (int, float)), data)


def get_rounded_nums(data: List) -> List[int]:
    """
    ...
    Returns:
        List: A List of integers with the fractional part removed.
    """
    numbers = filter_numbers(data)
    return list(map(int, numbers))


data = ['Timur', -16.648911695768902, 'six', -202,
        883.0093275936454, -765, (3, 4), -105.10718000213546,
        976, -308.96857946288094, 458, ['one', 'two'],
        479.92207220345927, -87, -71, 'twelve',
        112, -621, -715.0179551194733, 'seven',
        229, 729, -358, [1, 2, 3],
        -974, 882, -894.4709033242768, '',
        323.7720806756133, 'beegeek', -224, 431,
        170.6353248658936, -343.0016746052049, 'number',
