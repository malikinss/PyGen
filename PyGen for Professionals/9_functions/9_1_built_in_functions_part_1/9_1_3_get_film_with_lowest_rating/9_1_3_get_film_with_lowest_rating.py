...
film_ratings = {'Spider-Man: No Way Home': {'imdb': 8.8, 'kinopoisk': 8.3},
...
def calculate_average_ratings(films: dict[str, dict[str, float]]) -> dict[str, float]:
    """
    ...
    for film, rates in films.items():
        average_ratings[film] = sum(rates.values()) / len(rates)
...
def get_film_with_lowest_rating(films: dict[str, dict[str, float]]) -> str:
    ...
    # Find the film with the lowest average rating
